import { d, adjustForSlippage , initCetusSDK, Percentage, Pool, TickMath } from "@cetusprotocol/cetus-sui-clmm-sdk";
import { Injectable, Logger } from "@nestjs/common";
import { SwapParameters, SwapRequest, SwapResponse, SwapQuote } from "../../entities/cetus/swap.entity";
import { BN } from "bn.js";
import { CoinMetadata, SuiTransactionBlockResponse } from "@mysten/sui/dist/cjs/client";
...
export class CetusSwapService {
    ...
    private async getPoolByCoins(token_a: string, token_b: string): Promise<Pool | null> {
        try {
    ...
    private async getCoinMetadata(coinType: string): Promise<CoinMetadata | null> {
        try {
    ...
    private async calculateOptimalRoute(request: SwapRequest): Promise<SwapResponse> {
        try {
    ...
    private async executeSwap(quote: SwapQuote, signer): Promise<SuiTransactionBlockResponse> {

        const { poolAddress, coinTypeIn, coinTypeOut, amountIn, amountOut, a2b, byAmountIn, slippage } = quote.transactionData;
    ...
    async getTxHash(quote: SwapQuote, signer: string): Promise<SuiTransactionBlockResponse> {
        try {
