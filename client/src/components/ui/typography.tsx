...
interface TypographyProps<C extends React.ElementType> extends VariantProps<typeof typographyVariants> {
  as?: C;
...
export function Typography<C extends React.ElementType = "p">({
  ...
  return (
    <Component className={cn(typographyVariants({ variant, weight, align, color, size }), className)} {...props}>
      {children}
    </Component>
  );
}
