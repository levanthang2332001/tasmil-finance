...
const ModuleCard: React.FC<ModuleCardProps> = ({ module }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-1 -right-1 w-12 h-12 rotate-12 border-t-4 border-r-4 border-primary/20 rounded-tr-xl"></div>

      <h3 className="text-lg font-medium text-white relative z-10 flex items-center">
        <span className="w-2 h-8 bg-primary/80 mr-2"></span>
        {module.title}
        <span className="text-xs text-gray-400 ml-auto font-mono">{module.type}</span>
      </h3>
      ...
      <div className="flex-1 flex items-center justify-center relative z-10">
        ...
        <div className="w-full max-w-[80%] h-20 bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg flex items-center justify-center">
          <p className="text-gray-400 text-sm font-mono">// {module.title.toLowerCase()}_data</p>
        </div>
      </div>
